...
/// A utility class that automates the localization of the macOS app name.
///
...
/// modifying the `project.pbxproj` file to register these changes in Xcode for macOS.
class MacosProcessor {
  /// The main entry point for macOS localization processing.
  ///
  /// It takes a [config] map where keys are language codes and values are app names.
  /// It validates the macOS project structure and orchestrates the creation of
  /// localized string files and Xcode project file updates.
  static void process(Map<String, String> config) {
    print("💻 Processing macOS...");
    final String runnerPath = 'macos/Runner';
    final String projectFilePath = 'macos/Runner.xcodeproj/project.pbxproj';

    if (!Directory(runnerPath).existsSync()) {
      print("⚠️ macOS project not found at $runnerPath");
      return;
    }
...
''';
  ...
  static void _updatePbxProject(String projectPath, List<String> languages) {
    ...
    projectFile.writeAsStringSync(content);
    print("   ✅ Xcode (macOS) project updated successfully.");
  }

  /// Ensures the `InfoPlist.strings` variant group is linked to the Resources Build Phase.
  ///
  /// This allows Xcode to actually include the localized strings in the final macOS app bundle.
  static String _addToCopyBundleResources(String content, String fileRefUUID) {
    ...
    for (int phaseIdx in phaseIndices) {
      ...
      if (filesStart != -1 && filesEnd != -1) {
        ...
        // macOS usually uses AppIcon instead of LaunchScreen
        if (filesBlock.contains('Assets.xcassets') || filesBlock.contains('AppIcon')) {
    ...
    if (targetFilesIndex == -1) {
      print("   ❌ Error: Could not locate Main App Resources Phase for macOS.");
      return content;
    }
...
''';
  ...
  ///
  /// This boolean flag tells macOS to look for localized app names in `InfoPlist.strings`.
  static void _updateInfoPlist(String runnerPath) {
    ...
    if (!content.contains('LSHasLocalizedDisplayName')) {
      ...
      if (lastDictIndex != -1) {
        ...
        print("   ✅ Enabled LSHasLocalizedDisplayName for macOS.");
      }
    }
  }
  ...
  /// Reverts all localization changes made to the macOS project.
  ///
  ...
  static void revert() {
    print("⏪ Reverting macOS changes...");
    Utils.restoreBackup('macos/Runner.xcodeproj/project.pbxproj');
    Utils.restoreBackup('macos/Runner/Info.plist');
    print("   ✅ macOS revert complete.");
  }
}
